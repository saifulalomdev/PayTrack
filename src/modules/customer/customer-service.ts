...
/**
 * No password-like field exists on customer, so this is just an identity
 * pass-through — kept for symmetry with staffService.toSafeStaff so every
 * method below returns through the same "one safe shape" convention.
 */
...
export const customerService = {
     ...
     * staff's own session (see customer.actions.ts) — never trust a
     * client-supplied value here, since it's the only record left of who
     * added the customer once/if that staff account is later deleted.
     */
    ...
    /**
     * Admin only — role check is enforced in the action layer (requireAdmin),
     * not here. This layer only owns business rules (existence + uniqueness).
     */
